...
- [X] Explain the difference between `Relational Databases` and `SQL`.
    ...
    Relational databases are databases that store their data in tables. Structured Query Language is the standard language used to manage databases and the data within them (lets you search and manipulate data in a database). SQL and relational databases are completely different concepts. Relational databases just use SQL as the main interface for manipulating data but other database types (like key/value systems) can also support SQL



- [X] Why do tables need a `primary key`?

    A primary key is a way to identify each entry in the database uniquely. Other columns can have general/not unique names and to find exactly what we need  primary key is required. 
...
- [X] What do we need in order to have a _many to many_ relationship between two tables.
    ...
    We need intermediary table where foreign keys are connected to the certain primary keys. In this table primary key is the combination of two foreigns. 
